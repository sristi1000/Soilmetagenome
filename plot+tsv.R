...
# ============================================================
# 5. EXPORT COMPLETE, EXHAUSTIVE TABLES (DOWN TO 1 COUNT)
# ============================================================
...
write_csv(ko_abundance, file.path(tsv_dir, paste0(sample_name, "_KEGG_KO_abundance_all.csv")))
write_csv(pathway_abundance, file.path(tsv_dir, paste0(sample_name, "_KEGG_Pathway_abundance_all.csv")))
write_csv(module_abundance, file.path(tsv_dir, paste0(sample_name, "_KEGG_Module_abundance_all.csv")))
write_csv(cazy_abundance, file.path(tsv_dir, paste0(sample_name, "_CAZy_abundance_all.csv")))
write_csv(pfam_abundance, file.path(tsv_dir, paste0(sample_name, "_PFAM_abundance_all.csv")))

# Gene-level mapping export
gene_level_export <- df %>%
  select(query, seed_ortholog, evalue, score, COG_category, Preferred_name, KEGG_ko, KEGG_Pathway, KEGG_Module, CAZy, PFAMs) %>%
  arrange(desc(score))

write_csv(gene_level_export, file.path(output_dir, "02_functional_tables", paste0(sample_name, "_gene_level_annotations.csv")))

cat("Successfully exported full sorted tables down to 1 count.\n")
